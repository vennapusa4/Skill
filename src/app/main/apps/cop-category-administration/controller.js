(function () {
  ...
  angular
    ...
    .controller('MasterDataCopCategoryController', MasterDataCopCategoryController);

  /** @ngInject */
  function MasterDataCopCategoryController($scope, $timeout, MasterDataCopCategoryApi, $window, InsightsCommonService) {
    var vm = this;
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "CoPCategory", pageId);
    }
    $scope.pageSize = 10;
    $scope.Keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return MasterDataCopCategoryApi.getAllCopCategries(options, $scope.Keyword);
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          MasterDataCopCategoryApi.deleteItem({ id: e.data.id }).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      kendo.saveAs({ dataURI: workbook.toDataURL(), fileName: "CoPCategory.xlsx"});
    });
      ...
      save: function (e) {
        ...
        if (e.model.isNew()) {
          MasterDataCopCategoryApi.addNew(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          },function (error) {
          ...
          MasterDataCopCategoryApi.update(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          },function (error) {
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        MasterDataCopCategoryApi.deleteMultiCoPCategory(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
