(function () {
  ...
  angular
    ...
    .controller('AdminCopManagementController', AdminCopManagementController);

  /** @ngInject */
  function AdminCopManagementController($scope, AdminSettingCoPApi, $window, $rootScope ,  $state) {
    var vm = this;
    vm.category = [];
    vm.CopExcelData = [];
    ...
    $scope.getSearchResults = function () {
      ...
      vm.categoryList = [];
      $scope.keyword = $scope.searchfield;
      AdminSettingCoPApi.GetAllCoPCategory().then(function (res) {
        if(res != null){
          res.forEach(function (category){
            vm.categoryList.push(category); 
    ...
    $scope.$on("expandSearchedCategory", function(event, categoryID){
      if($scope.keyword != null || $scope.keyword != "" || typeof $scope.keyword !== undefined){
        $("#" + categoryID).css("height", "auto")
        $("#" + categoryID).addClass("in");
        $("#" + categoryID).attr("aria-expanded", "true");
        $("#" + categoryID).prev().attr("aria-expanded", "true");
        $("#" + categoryID).prev().removeClass("collapsed");
      }
    ...
    vm.currentCoP = [];
    $scope.$on("showAuditTrail", function (event, data) {
      vm.currentCoP = [];
      vm.currentCoP = data;
