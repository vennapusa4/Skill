...
(function () {
    ...
    angular.module('app.insights')
        .directive('insightsPeopleKnowledgeContribution', insightsPeopleKnowledgeContribution);
    /** @ngInject */
    function insightsPeopleKnowledgeContribution() {
        return {
            restrict: 'AE',
            replace: true,
            scope: {},
            controller: function ($scope, $filter, appConfig, InsightsCommonService, InsightsApi, $rootScope) {
                ...
                function loadData(segmentItems, isViewMore) {
                    ...
                    InsightsApi.getPeopleKnowledgeContribution(input).then(function (data) {
                        $scope.loading = false;
                        if (data != null) {
                            var maxValue = 0;
                ...
                function mappingToExcelData(data) {
                    $scope.exportData = [];
                    $scope.exportData.push(["#", "USERS", "KNOWLEDGE", "TOP CONTRIBUTOR", "VALUE"]);
                    var index = 1;
                    angular.forEach(data, function (cat, key1) {
                        $scope.exportData.push([cat.segmentType, null, null, null, null]);
                        angular.forEach(cat.items, function (item, key2) {
                            _.each(item.knowledgeContributionDepartmentResponses, function (x, key3) {
                                if (key3 == 0) {
                                    $scope.exportData.push([index, item.totalUser || 0, item.totalKnowledge || 0, x.name || '', x.value || 0]);
                                } else {
            ...
            templateUrl: 'app/main/apps/insights/_directives/people/insights-people-knowledge-contribution.html',
        };
    }
})();
