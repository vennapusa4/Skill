...
(function () {
    ...
    angular.module('app.insights')
        .directive('insightsCreatedMostValue', insightsCreatedMostValue);
    /** @ngInject */
    function insightsCreatedMostValue() {
        return {
            restrict: 'AE',
            replace: true,
            scope: {
            ...
            controller: function ($scope, $filter, appConfig, InsightsCommonService, InsightsApi, $rootScope) {
                    ...
                    InsightsApi.getValueCreation(input).then(function (data) {
                        ...
                        if (data != null) {
                            ...
                            _.each(data, function (item, index) {
                                _.each(item.valueCreationByDepartmentResponses, function (subitem, index2) {
                            ...
                            _.each(data, function (item, index) {
                                item.totalValue = InsightsCommonService.kmlFormatter(item.totalValue);
                            });
                ...
                function mappingToExcelData(data) {
                    $scope.exportData = [];
                    $scope.exportData.push(["#", "VALUE CREATION", "USERS", "KNOWLEDGE", "HIGHEST VALUE CREATED", "VALUE"]);
                    var index = 1;
                    angular.forEach(data, function (cat, key1) {
                        ...
                        angular.forEach(cat.items, function (item, key2) {
                            _.each(item.valueCreationByDepartmentResponses, function (x, key3) {
                                if (key3 == 0) {
                                    $scope.exportData.push([index, item.totalValue || 0, item.totalUser || 0, item.totalKnowledge || 0, x.name || '', x.value || 0]);
                                } else {
                                    $scope.exportData.push(['', '', '', '', x.name, x.value]);
                                }
                            });
                            index++;
                        });
            ...
            templateUrl: 'app/main/apps/insights/_directives/value/insights-created-most-value.html',
        };
    }
})();
