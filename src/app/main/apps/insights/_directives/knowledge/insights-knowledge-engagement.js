...
(function () {
    ...
    angular.module('app.insights')
        .directive('insightsKnowledgeEngagement', insightsKnowledgeEngagement);
    /** @ngInject */
    function insightsKnowledgeEngagement() {
        return {
            restrict: 'AE',
            replace: true,
            scope: {},
            controller: function ($scope, $filter, appConfig, InsightsCommonService, InsightsApi, $rootScope) {
                ...
                function loadData(segmentItems, isViewMore) {
                    ...
                    InsightsApi.getKnowledgeSubmission(input).then(function (data) {
                        $scope.loading = false;
                        var labels = [];
                        if (data != null) {
                            var result = _.chain(data).groupBy("submissionType").map(function (v, i) {
                                return {
                                    submissionType: i,
                                    items: v
                                }
            ...
            templateUrl: 'app/main/apps/insights/_directives/knowledge/insights-knowledge-engagement.html',
        };
    }
})();
