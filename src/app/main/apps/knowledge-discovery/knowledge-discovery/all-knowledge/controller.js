(function () {
    ...
    angular
        ...
        .controller('KnowledgeAllKnowledgeController', KnowledgeAllKnowledgeController);

    /** @ngInject */
    function KnowledgeAllKnowledgeController($scope, SearchApi, KnowledgeDocumentApi, InsightsCommonService, InsightsApi, MasterDataCopApi, $stateParams, $rootScope) {
        var vm = this;
        vm.loading = false;
        vm.arrSortby = [
        ...
        if ($rootScope.isShowValue == true) {
            $scope.IsShowValue = true;
            vm.isShowHasValue = true;
            $rootScope.isShowValue = false;
        }
        vm.isShowHasVideo = false;
        vm.isShowValidate = false;
        vm.isShowEndorsed = false;
        vm.isShowReplications = false;
        vm.switchMode = function (type) {
            ...
            if (type == 5) {
              vm.isShowEndorsed = !vm.isShowEndorsed;
            }
            if (type == 6) {
              vm.isShowReplications = !vm.isShowReplications;
            }
            vm.isShowAll = vm.isShowHasValue == false && vm.isShowHasVideo == false && vm.isShowValidate == false && vm.isShowEndorsed == false && vm.isShowReplications == false;
            getData();
        }
            ...
            select: function (e) {
                ...
                if (index == -1) {
                    $scope.$apply(function () {
                        vm.FilterSources[e.dataItem.realType].push(
                            {
                                id: e.dataItem.id,
                                itemGuid: e.dataItem.guid,
                                name: (e.dataItem.realType === 'Locations') ? (e.dataItem.type + ': ' + e.dataItem.text) : e.dataItem.text
                            }
                        );
                        vm.FilterSources[e.dataItem.realType] = vm.FilterSources[e.dataItem.realType].sort();
                        vm.Search();
                    });
                }
        ...
        vm.SearchText = function () {
            ...
            SearchApi.api.SearchSuggestionsText(postdata).then(function (data) {
                var arrType = Object.keys(vm.FilterSources).sort();//["Discipline", "Location", "Author"];
                _.each(arrType, function (type) {
                    ...
                    vm.FilterSources[type] = _.union(vm.FilterSources[type], lstObj).sort();
                });
                vm.Search();
            });
        };
        ...
        function getDataDocumentType() {
            ...
            function getFilter() {
                var results = [];
                var arrFilter = Object.keys(vm.FilterSources).sort();
        ...
        function getData() {
            ...
            KnowledgeDocumentApi.documentType(postData).then(function (data) {
                if (data != null && data.data != null) {
                    ...
                    var dataMap = [];
                    if (data != undefined && data != null && data.data != null && data.data.length > 0) {
