(function () {
  ...
  angular
    ...
    .controller('MasterDataDisciplinesController', MasterDataDisciplinesController);

  /** @ngInject */
  function MasterDataDisciplinesController($scope, $timeout, MasterDataDisciplineApi, $window, InsightsCommonService) {
    var vm = this;
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "Disciplines", pageId);
    }
    $scope.pageSize = 10;
    $scope.Keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return MasterDataDisciplineApi.getAll(options, $scope.Keyword);
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          MasterDataDisciplineApi.deleteItem({ id: e.data.id }).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      save: function (e) {
        ...
        if (e.model.isNew()) {
          MasterDataDisciplineApi.addDiscipline(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
        } else {
          MasterDataDisciplineApi.updateDiscipline(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        MasterDataDisciplineApi.deletemultiDiscipline(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
