(function () {
    ...
    angular
        ...
        .controller('MasterDataProgrammeController', MasterDataProgrammeController);

    /** @ngInject */
    function MasterDataProgrammeController($scope, $timeout, MasterDataProgrammeApi, $window) {
        var vm = this;
        $scope.pageSize = 10;
        $scope.Keyword = '';
        $scope.checkedIds = {};
        ...
        $scope.gridDataSource = new kendo.data.DataSource({
            transport: {
                read: function (options) {
                    ...
                    return MasterDataProgrammeApi.getAll(options, $scope.Keyword);
                },
                update: function (e) {
                    e.success();
                },
        ...
        $scope.mainGridOptions = {
            ...
            save: function (e) {
                ...
                if (e.model.isNew()) {
                    MasterDataProgrammeApi.addProgramme(formData).then(function (data) {
                        if (data.result) {
                            $scope.gridDataSource.read();
                        }
                    });
                } else {
                    MasterDataProgrammeApi.updateProgramme(formData).then(function (data) {
                        if (data.result) {
                            $scope.gridDataSource.read();
                        }
                    });
        ...
        $timeout(function () {
            ...
            $('.k-grid-delete').click(function (e) {
                ...
                MasterDataProgrammeApi.deletemultiProgramme(ids).then(function (data) {
                    if (data.result)
                        $scope.gridDataSource.read();
                });
            });
