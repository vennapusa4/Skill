(function () {
  ...
  angular
    ...
    .controller('MasterDataBusinessSectorController', MasterDataBusinessSectorController);

  /** @ngInject */
  function MasterDataBusinessSectorController($scope, $timeout, MDBusinessSectorAPI, $window, InsightsCommonService) {
    var vm = this;
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "BusinessSectors", pageId);
    }
    $scope.pageSize = 10;
    $scope.Keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return MDBusinessSectorAPI.getAll(options, $scope.Keyword, $scope.gridDataSource.total());
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          MDBusinessSectorAPI.deleteItem({ id: e.data.id }).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      excel: {
        allPages: true,
        filterable: true,
        fileName: "BusinessSectors.xlsx"
      },
      excelExport: function (e) {
        e.preventDefault();

        var data_columns = [
            { name: 'Category No.', width: 110 },
        ...
        var rows = [{
          cells: [
            { value: "Category No." ,background: "#7a7a7a",color: '#fff' , textAlign: "center"},
            { value: "CoP Category Name" ,background: "#7a7a7a",color: '#fff',},
          ]
        }];
      ...
      kendo.saveAs({ dataURI: workbook.toDataURL(), fileName: "BusinessSectors.xlsx"});
    });
      ...
      save: function (e) {
        ...
        if (e.model.isNew()) {
          MDBusinessSectorAPI.addBusinessSector(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          },function (error) {
          ...
          MDBusinessSectorAPI.updateBusinessSector(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          },function (error) {
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        var confirm = $window.confirm("Are you sure you want to remove this record?");
        if (!confirm) {
          return;
        }
        ...
        MDBusinessSectorAPI.deleteMultiBusinessSectors(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
