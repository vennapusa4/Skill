(function () {
    ...
    angular
        ...
        .controller('MasterDataIdeasCategoryController', MasterDataIdeasCategoryController);

    /** @ngInject */
    function MasterDataIdeasCategoryController($scope, $timeout, MasterDataIdeasCategoryApi, $window) {
        ...
        $scope.gridDataSource = new kendo.data.DataSource({
            transport: {
                read: function (options) {
                    ...
                    return MasterDataIdeasCategoryApi.getAll(options, $scope.Keyword);
                },
                update: function (e) {
                    e.success();
                },
        ...
        $scope.mainGridOptions = {
            ...
            toolbar: [
                ...
                allPages: true,
                filterable: true,
                fileName: "Ideas Category.xlsx"
            },
            excelExport: function (e) {
                var sheet = e.workbook.sheets[0];
                _.set(sheet, 'columns', [
                    { width: 300, autoWidth: false },
                    { width: 350, autoWidth: false },
                ]);
            },
            ...
            save: function (e) {
                ...
                if (e.model.isNew()) {
                    MasterDataIdeasCategoryApi.addIdeasCategory(formData).then(function (data) {
                        if (data.result) {
                            $scope.gridDataSource.read();
                        }
                    });
                } else {
                    MasterDataIdeasCategoryApi.updateIdeasCategory(formData).then(function (data) {
                        if (data.result) {
                            $scope.gridDataSource.read();
                        }
                    });
        ...
        $timeout(function () {
            ...
            $('.k-grid-delete').click(function (e) {
                ...
                MasterDataIdeasCategoryApi.deletemultiIdeasCategory(ids).then(function (data) {
                    if (data.result)
                        $scope.gridDataSource.read();
                });
            });
