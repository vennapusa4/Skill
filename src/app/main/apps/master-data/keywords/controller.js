(function () {
  ...
  angular
    ...
    .controller('MasterDataKeywordsController', MasterDataKeywordsController);

  /** @ngInject */
  function MasterDataKeywordsController($scope, $timeout, KeywordApi, $window, InsightsCommonService) {
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "Keywords", pageId);
    }
    $scope.pageSize = 10;
    $scope.keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return KeywordApi.getAll(options, $scope.keyword,$scope.gridDataSource.total());
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          KeywordApi.deleteItem(e.data.id).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      save: function (e) {
        ...
        var formData = {
          "id": e.model.id,
          "keywordName": e.model.name,
          "isDeleted": false
        };
        if (e.model.isNew()) {
          KeywordApi.addKeyword(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
        } else {
          KeywordApi.updateKeyword(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        KeywordApi.deleteMultiKeyword(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
