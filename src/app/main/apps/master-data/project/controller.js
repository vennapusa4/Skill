(function () {
  ...
  angular
    ...
    .controller('MasterDataProjectController', MasterDataProjectController);

  /** @ngInject */
  function MasterDataProjectController($scope, $timeout, ProjectApi, $window, InsightsCommonService) {
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "Projects", pageId);
    }
    $scope.pageSize = 10;
    $scope.keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return ProjectApi.getAll(options, $scope.keyword);
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          ProjectApi.deleteItem(e.data.projectId).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      save: function (e) {
        ...
        if (e.model.isNew()) {
          ...
          ProjectApi.addProject(formData1).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
          ...
          ProjectApi.updateProject(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        ProjectApi.deleteMultiProject(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
