(function () {
  ...
  angular
    ...
    .controller('MasterDataEquipmentController', MasterDataEquipmentController);

  /** @ngInject */
  function MasterDataEquipmentController($scope, $timeout, MasterDataEquipmentApi, $window, InsightsCommonService) {
    var vm = this;
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "Equipments", pageId);
    }
    $scope.pageSize = 10;
    $scope.Keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return MasterDataEquipmentApi.getAll(options, $scope.Keyword);
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          MasterDataEquipmentApi.deleteItem({ id: e.data.id }).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      columns: [
        ...
        },
        {
          ...
          template: function (dataItem) {
            return "<span style='cursor: pointer' ng-click='ApplyFilterFromMasterData(dataItem,1)'>" + dataItem.sumLessonsLearnt + "</span>";
          }
        },
        {
      ...
      save: function (e) {
        ...
        if (e.model.isNew()) {
          MasterDataEquipmentApi.addNew(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
        } else {
          MasterDataEquipmentApi.update(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        MasterDataEquipmentApi.deleteMultiEquipment(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
