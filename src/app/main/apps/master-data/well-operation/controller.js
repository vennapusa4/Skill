(function () {
  ...
  angular
    ...
    .controller('MasterDataWellOperationController', MasterDataWellOperationController);

  /** @ngInject */
  function MasterDataWellOperationController($scope, $window, $timeout, MasterDataWellOperationApi, InsightsCommonService) {
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "WellsOperations", pageId);
    }
    $scope.pageSize = 10;
    $scope.keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return MasterDataWellOperationApi.getAll(options, $scope.keyword);
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          MasterDataWellOperationApi.deleteWellOperation(e.data.id).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      save: function (e) {
        ...
        if (e.model.isNew()) {
          MasterDataWellOperationApi.addWellOperation(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
        } else {
          MasterDataWellOperationApi.updateWellOperation(formData).then(function (data) {
            if (data.result) {
              $scope.gridDataSource.read();
            }
          });
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        MasterDataWellOperationApi.deleteMultiWellOperation(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
