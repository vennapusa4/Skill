(function () {
  ...
  angular
    ...
    .controller('MasterDataWellPhaseController', MasterDataWellPhaseController);

  /** @ngInject */
  function MasterDataWellPhaseController($scope, $window, $timeout, MasterDataWellPhaseApi, InsightsCommonService) {
    $scope.ApplyFilterFromMasterData = function (item, pageId) {
      InsightsCommonService.applyFilterForMasterData(item, "WellsPhase", pageId);
    }
    $scope.pageSize = 10;
    $scope.keyword = '';
    $scope.checkedIds = {};
    ...
    $scope.gridDataSource = new kendo.data.DataSource({
      transport: {
        read: function (options) {
          ...
          return MasterDataWellPhaseApi.getAll(options, $scope.keyword);
        },
        update: function (e) {
          e.success();
        },
        destroy: function (e) {
          MasterDataWellPhaseApi.deleteWellPhase(e.data.wellPhaseId).then(function (data) {
            if (data.result) {
    ...
    $scope.mainGridOptions = {
      ...
      save: function (e) {
        ...
        var formData = {
          id: e.model.id,
          name: e.model.wellPhaseName,
          isDeleted: false
        };
        if (e.model.isNew()) {
          MasterDataWellPhaseApi.addWellPhase(formData).then(function (data) {
            if (data.result)
              $scope.gridDataSource.read();
          });
        } else {
          MasterDataWellPhaseApi.updateWellPhase(formData).then(function (data) {
            if (data.result)
              $scope.gridDataSource.read();
          });
        }
    ...
    $timeout(function () {
      ...
      $('.k-grid-delete').click(function (e) {
        ...
        MasterDataWellPhaseApi.deleteMultiWellPhase(ids).then(function (data) {
          if (data.result)
            $scope.gridDataSource.read();
        });
      });
